...
export class MandelbrotTricornPerturbation {
            ...
            for (let x = 0; x < w; x++) {
                if (skipLeft && x % 2 === 0) {
                    ...
                    for (let refIndex = 0; refIndex < numRefs; refIndex++) {
                        ...
                        const iter = this.tricorn_perturbation(dr - refDr, di - refDi, this.max_iter, bailout, referencePoint[3], referencePoint[4])
                        if (iter >= 0) {
     ...
     * @returns {number} iter
     */
    tricorn_perturbation(dcr, dci, max_iter, bailout, zs, numZs) {
        // ε₀ = δ
        let u = dcr
        let v = dci
    ...
    calculate_reference(refr, refi, dr, di, bigScale, scaleFactor, bailout) {
        ...
        const [iter, zq, seq] = this.tricorn_high_precision(rr, ri, this.max_iter, bailout, bigScale)
     ...
     */
    tricorn_high_precision(re, im, max_iter, bailout, scale) {
